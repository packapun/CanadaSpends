...
const ontarioData = JSON.parse(fs.readFileSync('./Ontario2024CompactSankey.json', 'utf8'));
    ...
    function traverse(items, parentPath = '') {
        ...
        for (const item of items) {
            if (item.amount !== undefined) {
                ...
                let categoryName = item.name.replace(`${ministryName} → `, '').replace(`${ministryName} \u2192 `, '');
...
ministries.forEach(ministry => {
    ...
    if (ministry.amount !== undefined) {
        ...
        totalSpending = ministry.children ? calculateTotalSpending(ministry.children) : 0;
    } else {
        totalSpending = calculateTotalSpending(children);
    }
    
    // Flatten the data for mini-sankey
    const categories = children ? flattenMinistryData(children, ministryName) : [];
    ...
    console.log(`✓ Created ${filename} - ${ministryName}: ${ministryData.totalSpendingFormatted} (${ministryData.percentageFormatted})`);
});
...
fs.writeFileSync(path.join(ministriesDir, '_summary.json'), JSON.stringify(summaryData, null, 2));

console.log('\n📊 Ministry Spending Summary (Top 10):');
ministryStats.slice(0, 10).forEach((ministry, index) => {
    console.log(`${index + 1}. ${ministry.name}: ${ministry.totalSpendingFormatted} (${ministry.percentageFormatted})`);
});
